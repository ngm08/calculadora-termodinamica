...
if calc_btn:
    ...
    if isinstance(Z, list):
        if P_prop > Pc:
            Z_final = Z[0]
            st.info(f"Fase Líquida (Z={Z_final:.4f})")
        else:
            Z_final = Z[-1]
            st.info(f"Fase Vapor (Z={Z_final:.4f})")
    else:
        Z_final = Z
...
with t_col1:
    ...
    P_in_t = st.number_input("Pressão de Entrada (bar)", value=60.0)
    T_in_t = st.number_input("Temperatura de Entrada (K)", value=650.0)
