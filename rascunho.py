...
st.set_page_config(page_title="Calculadora Termodinâmica", layout="centered")
...
def nrz(f, df, Z_chute, A, B, max_iter=200):
    ...
    while i < max_iter:
        den = df(Z_old,A,B)
        if den == 0: return None # Evitar divisão por zero
...
def h_res(Z, T, P, Pc, Tc, w):
    ...
    # Proteção matemática para log negativo
...
def calc_Z(A, B, P, Pc):
    raizes = []
    for chute in [i/20 for i in range(1, 21)]: # Começar de 1 para evitar Z=0
        raiz = nrz(f, df, chute, A, B)
        if raiz == None:
            continue
        if not any(abs(raiz - r) < 1e-8 for r in raizes):
            if raiz > 0: # Z deve ser positivo
                raizes.append(raiz)

    Z_lista = sorted(raizes)
    
    if not Z_lista: return 1.0 # Fallback se não achar raiz

    if len(Z_lista) == 1:
        return Z_lista[0]
    else:
        # Se houver mais de uma, retorna a lista para decisão baseada em fase
        return Z_lista
...
with col1:
    st.subheader("Informações da Substância")
    composto = st.selectbox("Escolha o Composto:", list(dados.keys()))
    P = st.number_input("Pressão de análise (bar):", value=10.0, min_value=0.1)
    T = st.number_input("Temperatura de análise (K):", value=350.0, min_value=100.0)

# Dados do composto selecionado
Pc, Tc, w, Cp_coefs = dados[composto].values()
Tref= 273.15 
Pref= 1.01325 

# Botão para calcular propriedades de estado
if st.button("Calcular Propriedades"):
    A, B, *_ = AB(P, T, Pc, Tc, w)
    Z = calc_Z(A, B, P, Pc)
    
    # Lógica de seleção de raiz (mantida do seu código)
    ...
    # Cálculos
    vm = (Z_final * R * T / P)*1e3
    h_val = h_calc(Z_final, T, P, Tref, Pref, Pc, Tc, Cp_coefs, w)
    s_val = s_calc(Z_final, T, P, Tref, Pref, Pc, Tc, Cp_coefs, w)

    # Exibição bonita com st.metric
    c1, c2, c3 = st.columns(3)
    c1.metric("Volume Molar (L/mol)", f"{vm:.4f}")
    c2.metric("Entalpia (J/mol)", f"{h_val:.2f}")
    c3.metric("Entropia (J/mol.K)", f"{s_val:.4f}")

st.markdown("---")

# --- SEÇÃO DA TURBINA ---
with st.expander("Simulação de Turbina (Expansão)", expanded=False):
    st.write("Configuração da turbina usando o composto selecionado acima.")
    
    t_col1, t_col2 = st.columns(2)
    with t_col1:
        P_in_t = st.number_input("Pressão Entrada (bar)", value=60.0)
        T_in_t = st.number_input("Temp. Entrada (K)", value=650.0)
        taxa = st.number_input("Taxa de fluxo (mol/s)", value=20.0)
    with t_col2:
        P_out_t = st.number_input("Pressão Saída (bar)", value=10.0)
        eff = st.slider("Eficiência Isentrópica", 0.0, 1.0, 0.8)

    if st.button("Simular Turbina"):
        with st.spinner('Iterando solução...'):
            try:
                pot, T_out_res, s_ger = turbina(P_in_t, T_in_t, P_out_t, eff, taxa, Pref, Tref, composto)
                
                res1, res2, res3 = st.columns(3)
                res1.metric("Potência Gerada", f"{pot:.2f} kW")
                res2.metric("Temp. de Saída", f"{T_out_res:.2f} K")
                res3.metric("Entropia Gerada", f"{s_ger:.4f} kJ/K")
            except Exception as e:
                st.error(f"Erro na convergência numérica: {e}. Tente outros valores de P ou T.")